...
def save_file_to_csv(results):
    results.to_csv(args.output_entropy, header = True, index = False, sep = '\t')
...
def distribute_datasets(dataset):
    ...
    #number of batches
    kwargs['N_batches'] = len(dataset.obs[args.batch_key].astype('category').cat.categories)
    ...
    kwargs['N_cell_types'] = len(dataset.obs[args.celltype_key].astype('category').cat.categories)    
